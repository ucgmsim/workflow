...
def test_analytic_gain_matches_qcore(
    ...
    np.testing.assert_allclose(analytic, measured_response(band, frequencies), atol=2e-3)
...
def test_no_source_filter_is_a_no_op() -> None:
    ...
    lf_gain, hf_gain = recombination_gains(
        frequencies, DT, FLO, None, CorrectionLeg.LF
    )
...
def test_lf_restores_the_power_sum_where_the_lf_leg_matters(
    ...
    assert power_sum_error(source, CorrectionLeg.LF, np.linspace(*BAND, 400)).max() < 0.01
...
def test_solver_filters_are_read_from_the_realisation(tmp_path: Path) -> None:
    ...
    version = RealisationMetadata.read_from_realisation(realisation_ffp).defaults_version
