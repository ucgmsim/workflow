...
def butterworth_gain(
    ...
    """
    sos = sp.signal.butter(order, corner, btype=band, output="sos", fs=1.0 / dt)
...
def recombination_gains(
    ...
    match leg:
        case CorrectionLeg.LF:
            ...
            # The denominator is floored rather than the quotient clipped: the
            # two are identical for a non-negative gain, but this does not go
            # via an intermediate inf where the source filter has rolled off
            # to zero.
            return 1.0 / np.maximum(source_gain, 1.0 / MAX_BOOST), ones
        case CorrectionLeg.HF:
            ...
            hf_gain = np.where(
                ...
                np.clip(
                    shortfall / np.maximum(highpass, HF_GAIN_FLOOR), None, MAX_BOOST
                ),
                1.0,
            )
            return ones, hf_gain
...
def combine_hf_and_lf(
    ...
    warn_if_ill_conditioned(bb_dt, broadband_config.flo, source_lowpass, filter_leg)
